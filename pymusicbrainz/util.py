...
def flatten_title(artist_name="", recording_name="", album_name="") -> str:
    ...
    return unidecode(
        re.sub(r"\W+", "", artist_name + album_name + recording_name).lower()
    )
    ...
    with db.get_db_session() as session:
        ...
        if area.type_id is not None and area.type_id != 1:
            stmt = (
                ...
                .join_from(
                    mbdata.models.AreaContainment,
                    mbdata.models.Area,
                    mbdata.models.AreaContainment.parent,
                )
                .where(mbdata.models.AreaContainment.descendant == area)
...
def parse_partial_date(
    ...
    if isinstance(partial_date, mbdata.models.PartialDate):
        ...
        if partial_date.day is None:
            return datetime.date(
                year=partial_date.year, month=partial_date.month, day=1
            )
        return datetime.date(
